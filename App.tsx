...
const App = () => {
  ...
  const handleReveal = async () => {
    ...
    try {
      // Optica Platinum v1.7.0 prompt injection
  ...
  const downloadHighRes = async (imageBase64: string, id: string) => {
    try {
      setToast("Fijando negativo maestro...");
      const res = await fetch(imageBase64);
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `FOTOGRAFO_MASTER_${id}.png`;
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-zinc-400 font-sans flex overflow-hidden selection:bg-red-500 selection:text-white">
      {toast && (
        <div className="fixed bottom-12 left-1/2 -translate-x-1/2 z-[300] bg-amber-600 text-black px-6 py-2.5 rounded-sm font-mono font-bold uppercase tracking-widest text-[10px] shadow-[0_0_40px_rgba(245,158,11,0.4)] animate-in fade-in slide-in-from-bottom-4 duration-500 border border-amber-500/50">
          {toast}
        </div>
      )}

      {/* Sensor Control Hub (Studio Logic) */}
      <div className="fixed top-6 right-6 z-[100] flex flex-col items-end gap-2">
        ...
        {engineMode === 'live' && !hasGeminiKey && (
          <button onClick={handleSelectKey} className="text-[8px] font-mono font-bold uppercase tracking-widest text-amber-500 bg-amber-500/5 px-2 py-1 rounded border border-amber-500/20 animate-pulse">
            Calibrar Sensor Pro
          </button>
        )}
      </div>
      
      {/* Navigation (Leica Aesthetic) */}
      <nav className="hidden sm:flex w-20 bg-black/40 border-r border-white/5 flex-col items-center py-10 space-y-12 z-50 hardware-panel">
        ...
        {[
          { m: ViewMode.ANALYSIS, icon: <ICONS.ANALYSIS />, t: 'SUJETO (RAW)' },
          { m: ViewMode.ARCHITECT, icon: <ICONS.ARCHITECT />, t: 'ESQUEMA' },
          { m: ViewMode.DARKROOM, icon: <ICONS.DARKROOM />, t: 'LABORATORIO' },
          { m: ViewMode.RENDER_VAULT, icon: <ICONS.RENDER_VAULT />, t: 'ARCHIVO' },
          { m: ViewMode.ANALYTICS, icon: <ICONS.ANALYTICS />, t: 'BITÁCORA' }
        ].map(item => (
          <button key={item.m} onClick={() => setView(item.m)} className={`group relative p-3 rounded-xl transition-all duration-300 ${view === item.m ? 'text-amber-500 bg-amber-500/5 shadow-[0_0_25px_rgba(245,158,11,0.15)] border border-amber-500/20' : 'text-zinc-700 hover:text-zinc-400 hover:bg-white/5'}`}>
             {item.icon}
      ...
      <main className="flex-1 overflow-y-auto p-6 sm:p-12 custom-scrollbar relative">
        <header className="flex flex-col mb-16 sm:mb-20">
          <div className="flex justify-between items-end">
            <div className="flex flex-col">
              <h1 className="text-xl font-mono font-black italic text-zinc-100 uppercase tracking-tighter">EL FOTÓGRAFO: <span className="text-amber-600 font-normal">Studio Master</span></h1>
              <span className="text-[8px] font-mono font-bold text-zinc-700 tracking-[0.4em] uppercase">v1.7.0 Platinum Core</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-[8px] font-mono font-bold text-zinc-600 uppercase tracking-widest">Estado Sensor:</span>
              <div className={`w-1.5 h-1.5 rounded-full ${engineMode === 'live' ? 'bg-red-500 animate-pulse shadow-[0_0_8px_rgba(255,59,48,0.8)]' : 'bg-zinc-700'}`}></div>
            </div>
          </div>
        ...
        {view === ViewMode.ANALYSIS && (
          <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div className="text-center space-y-4">
              <Badge color="amber">EXPOSICIÓN DE NEGATIVO</Badge>
              <h2 className="text-3xl sm:text-5xl font-mono font-black text-zinc-100 italic uppercase tracking-tighter">Cargar <span className="text-amber-600">RAW / Negativo</span></h2>
              <p className="text-zinc-600 text-[9px] font-mono font-bold uppercase tracking-[0.2em]">Paso 1: Captura de la firma facial para el proceso de revelado.</p>
            </div>
            
            <div className="h-[300px] sm:h-[400px] border-2 border-dashed border-zinc-800 rounded-3xl bg-black/20 flex flex-col items-center justify-center p-12 group hover:border-amber-600/30 transition-all cursor-pointer relative overflow-hidden" onClick={() => fileInputRef.current?.click()}>
              ...
              {loading ? (
                <div className="flex flex-col items-center space-y-6">
                  <div className="w-12 h-12 border-b-2 border-amber-600 rounded-full animate-spin" />
                ...
                <div className="text-center space-y-6">
                  <div className="w-16 h-16 rounded-full bg-amber-600/5 flex items-center justify-center mx-auto text-amber-500 group-hover:scale-105 transition-transform border border-amber-500/10"><ICONS.ANALYSIS /></div>
                  <p className="text-[10px] font-mono font-bold uppercase text-zinc-600 tracking-[0.2em]">Seleccionar Negativo Maestro</p>
                </div>
              )}
        ...
        {view === ViewMode.ARCHITECT && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in fade-in duration-500 pb-24 sm:pb-0">
            <div className="lg:col-span-4 space-y-8">
              <Card title="Firma Facial (ADN)" subtitle={state.currentSession.facialId || "PENDIENTE"}>
              ...
              <Card title="Equipo Óptico (Gear)" subtitle="Studio Gear Rack">
                <GearRack 
            ...
            <div className="lg:col-span-8 space-y-8">
               <Card title="Esquema de Pose">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                    {state.poseBank.map(p => (
                      <button key={p.id} onClick={() => setState(s => ({ ...s, currentSession: { ...s.currentSession, currentPose: p } }))} className={`p-5 rounded-lg border text-left transition-all ${state.currentSession.currentPose?.id === p.id ? 'bg-amber-100 text-black border-amber-100' : 'bg-black/20 border-zinc-800 text-zinc-500 hover:bg-black/30'}`}>
               ...
               <Card title="Locación del Set">
                <input type="text" value={state.currentSession.currentContext} onChange={(e) => setState(s => ({ ...s, currentSession: { ...s.currentSession, currentContext: e.target.value } }))} placeholder="Penthouse, estudio minimalista, exterior noche..." className="w-full bg-black/40 border border-zinc-800 rounded-md p-4 text-[10px] font-mono outline-none focus:border-amber-900/50 transition-colors" />
               </Card>
               
               <div className="flex justify-end pt-4">
                <Button onClick={() => setView(ViewMode.DARKROOM)} className="h-16 px-12 text-xs" variant="shutter">Entrar al Laboratorio</Button>
               </div>
            </div>
          </div>
        )}
        ...
        {view === ViewMode.DARKROOM && (
          <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-500 pb-24 sm:pb-0">
            <Card title={`Laboratorio Químico-Digital — Sensor ${engineMode === 'live' ? 'Full Frame' : 'Native'}`} className="h-[500px] sm:h-[600px] relative overflow-hidden group">
              {darkroomLoading ? (
                <div className="absolute inset-0 bg-black/95 flex flex-col items-center justify-center space-y-10 z-50">
                  <div className="w-20 h-20 border border-amber-600/20 rounded-full flex items-center justify-center text-amber-600 shadow-[0_0_30px_rgba(245,158,11,0.1)]"><ICONS.DARKROOM /></div>
                  <div className="text-center space-y-4">
                    <p className="text-zinc-100 font-mono font-black italic uppercase tracking-[0.4em] text-lg animate-pulse">REVELANDO GRANO FINO...</p>
                    {retryInfo && <Badge color="amber">Ajuste Biométrico: {retryInfo.score}%</Badge>}
                  </div>
                </div>
              ) : (
                <div className="h-full flex flex-col items-center justify-center p-12 text-center space-y-10">
                  <div className="w-24 h-24 rounded-full bg-amber-600/5 border border-amber-600/10 flex items-center justify-center mx-auto text-amber-600 scale-125 mb-4 shadow-[0_0_50px_rgba(245,158,11,0.05)]"><ICONS.DARKROOM /></div>
                  <div className="space-y-4">
                    <h3 className="text-4xl font-mono font-black italic text-zinc-100 uppercase tracking-tighter">DISPARAR <span className="text-amber-600">REVELADO</span></h3>
                    <p className="text-zinc-600 text-[9px] font-mono font-bold uppercase tracking-[0.2em] max-w-sm mx-auto leading-relaxed">
                      Lente: {state.currentSession.gearSettings.focalLength} | Película: {state.currentSession.gearSettings.filmStock}
                    </p>
                  </div>
                  <Button onClick={handleReveal} variant="shutter" className="h-24 px-24 text-xl shadow-[0_0_60px_rgba(255,59,48,0.2)]">DISPARAR</Button>
                </div>
              )}
            </Card>
          </div>
            ...
            <div className="space-y-10">
              <div className="flex justify-between items-end border-b border-zinc-900 pb-6">
                <div className="flex flex-col">
                  ...
                  <span className="text-[8px] font-mono text-zinc-700 uppercase tracking-widest mt-1">Negativos Originales Archivados</span>
                </div>
                <Badge color="amber">SAFE STORAGE</Badge>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {state.savedRenders.map(r => (
                  <Card key={r.id} title={r.id.split('_')[1]} subtitle={new Date(r.createdAt).toLocaleDateString()}>
                    ...
                    {/* EXIF Metadata Strip */}
                    <div className="flex justify-between items-center mb-5 px-2 bg-black/40 py-2 rounded border border-white/5 mono text-[7px] uppercase tracking-tighter text-zinc-500">
                      <div>L: {r.metadata?.gear?.focalLength} | S: {r.metadata?.engineMode}</div>
                      <div>P: {r.metadata?.gear?.filmStock}</div>
                    </div>

                    <div className="flex flex-col gap-2">
                      <Button onClick={() => downloadHighRes(r.outBase64!, r.id)} variant="primary">Exportar Master</Button>
            ...
            <div className="space-y-10">
              <div className="flex justify-between items-end border-b border-zinc-900 pb-6">
                <div className="flex flex-col">
                  ...
                  <span className="text-[8px] font-mono text-zinc-800 uppercase tracking-widest mt-1">Tomas Recientes del Laboratorio</span>
                </div>
                <Button variant="secondary" className="text-[8px] py-1 px-3" onClick={() => setState(p => ({...p, renderLibrary: []}))}>Limpiar Carrete</Button>
              </div>
                ...
                {state.renderLibrary.map(r => (
                  <Card key={r.id} title={r.id.split('_')[1]} subtitle={r.metadata?.usedModel || "RAW"}>
                    <div className="aspect-[3/4] rounded-lg overflow-hidden mb-3 border border-zinc-900 bg-zinc-950 group relative">
                      <img src={r.outBase64} className="w-full h-full object-cover grayscale transition-all duration-700 group-hover:grayscale-0" />
                      <div className="absolute bottom-3 left-3"><Badge color="red">Match: {r.metadata?.identityValidation?.matchScore}%</Badge></div>
                    </div>

                    {/* EXIF Metadata Strip */}
                    <div className="flex justify-between items-center mb-5 px-2 bg-black/40 py-2 rounded border border-white/5 mono text-[7px] uppercase tracking-tighter text-zinc-600">
                      <div>Lens: {r.metadata?.gear?.focalLength} | Sensor: {r.metadata?.engineMode}</div>
                      <div>Film: {r.metadata?.gear?.filmStock}</div>
                    </div>
        ...
        {view === ViewMode.ANALYTICS && (
          <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in duration-700 pb-24">
            <div className="text-center space-y-4">
                <Badge color="amber">BITÁCORA DE TELEMETRÍA EXIF</Badge>
                <h2 className="text-5xl font-mono font-black text-zinc-100 italic uppercase tracking-tighter">Métricas del <span className="text-amber-600">Sensor</span></h2>
                <p className="text-zinc-600 text-[9px] font-mono font-bold uppercase tracking-[0.2em]">Rendimiento técnico y calidad de los revelados químicos.</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
                <Card title="Disparos" subtitle="TOTAL">
                    <p className="text-4xl font-mono font-black text-white">{stats.totalGenerations}</p>
                </Card>
                <Card title="Calidad Avg" subtitle="MATCH FIDELITY">
                    <p className="text-4xl font-mono font-black text-amber-600">{Math.round(stats.averageMatchScore)}%</p>
                </Card>
                <Card title="Protocolos" subtitle="INTENTOS RETRY">
                    <p className="text-4xl font-mono font-black text-white">{stats.averageAttempts.toFixed(1)}</p>
                </Card>
                <Card title="Éxito" subtitle="VALIDADOS">
